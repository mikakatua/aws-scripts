...
do
  ...
  then
    ...
    echo -e "\n\033[1m$line:\033[0m"
    aws logs get-log-events --log-group-name $LOG_GROUP --log-stream-name $LOG_STREAM --query "events[*].[timestamp,message]" $GETLOGEVENTS_ARGS --output text | sed '/^$/d' | awk '{$1=strftime("%Y-%m-%d %H:%M:%S", $1/1000); print $0}'
  fi
done < <(aws logs describe-log-streams --log-group-name $LOG_GROUP --query "logStreams[*].[logStreamName,firstEventTimestamp,lastEventTimestamp]" --output text | sort -n -k 2)
